...
export default function OrderDetails({ order, onViewBank, showNextSteps = true }) {
  return <article className="order-detail-card">
    ...
    <div className="order-detail-sections">
      ...
      <section><h3>PAYMENT</h3>{isPaymentPending(order) && <strong className="payment-required">{paymentRequirement(order)}</strong>}<dl className="order-detail-list"><div><dt>Payment method</dt><dd>{order.payment_method}</dd></div><div><dt>Payment state</dt><dd>{order.payment_status}</dd></div><div><dt>Subtotal</dt><dd>{formatProductPrice(Number(order.subtotal_amount))}</dd></div>{order.coupon_code && <><div><dt>Coupon</dt><dd>{order.coupon_code}</dd></div><div><dt>Discount</dt><dd>− {formatProductPrice(Number(order.discount_amount))}</dd></div></>}<div><dt>Order total</dt><dd>{formatProductPrice(Number(order.total_amount))}</dd></div><div><dt>Pay now</dt><dd>{formatProductPrice(Number(order.advance_amount))}</dd></div><div><dt>Paid</dt><dd>{formatProductPrice(Number(order.paid_amount))}</dd></div><div><dt>Balance on delivery</dt><dd>{formatProductPrice(Number(order.pending_amount))}</dd></div></dl>{onViewBank && isPaymentPending(order) && <button className="btn btn--acid order-bank-button" type="button" onClick={onViewBank}>VIEW BANK DETAILS <span>↗</span></button>}</section>
