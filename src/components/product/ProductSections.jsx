  ...
  return (
    <section className="gallery reveal" aria-label={`${product.name} image gallery`}>
      ...
      <div className="gallery-main">
        ...
        <img className={imageVisible ? '' : 'image-hidden'} src={displayedImage} alt={`${product.brand} ${product.name}`} />
      </div>
    </section>
  );
}
...
export function ProductInfo({ product, selectedSize, setSelectedSize, payment, setPayment, addToBag }) {
  ...
  const amount = payment === 'deposit' ? `${formatPrice(deposit)} DEPOSIT` : formatPrice(product.price);
  ...
  return (
    <section className="product-info reveal delay-100">
      <div className="product-flags">
        <span className="badge badge--acid badge--static">{product.availability.toUpperCase()}</span>
      ...
      <p className="product-subtitle">{product.colorway} · Style {product.styleCode}</p>
      <div className="product-rating"><span>★★★★★</span><strong>4.9</strong><span>Verified KICKZ.LK sourcing</span></div>
      <div className="product-price"><strong>{formatPrice(product.price)}</strong><span>Taxes and import handling included</span></div>
      ...
      {product.preOrder && (
        <div className="option-section">
          ...
          <div className="payment-options">
            <label className="payment-card"><input type="radio" name="payment" value="deposit" checked={payment === 'deposit'} onChange={(event) => setPayment(event.target.value)} /><span><strong>Pay deposit</strong><small>{formatPrice(deposit)} today</small></span></label>
            <label className="payment-card"><input type="radio" name="payment" value="full" checked={payment === 'full'} onChange={(event) => setPayment(event.target.value)} /><span><strong>Pay in full</strong><small>{formatPrice(product.price)} today</small></span></label>
          </div>
        </div>
      )}
      <div className="delivery-box"><b>↗</b><div><strong>Estimated delivery: {product.deliveryTime}</strong><span>Regular WhatsApp updates from sourcing to islandwide delivery.</span></div></div>
...
export function ProductDetails({ product }) {
  ...
  const tabs = [
    ...
    { id: 'delivery', label: 'DELIVERY & PRE-ORDER', content: <><h3>Clear delivery timeline.</h3><p>{product.preOrder ? 'Secure the order with the selected deposit. KICKZ.LK confirms sourcing, shares progress through WhatsApp, and collects the remaining balance before islandwide dispatch.' : 'This pair is currently available for local fulfilment. KICKZ.LK confirms the order and shares dispatch progress through WhatsApp.'} Estimated delivery is {product.deliveryTime}.</p></> },
