...
export default function CheckoutPage() {
  ...
  return <PageShell>
    <PageHero kicker="SECURE ORDER" title="CHECKOUT" copy="Confirm delivery details, payment amounts and your bank-transfer reference." />
    <section className="checkout-section section-pad"><form className="container checkout-layout" onSubmit={placeOrder} noValidate>
      <div className="checkout-form customer-form">
        <div className="checkout-heading"><span className="section-kicker">{accountCustomer ? 'ACCOUNT CHECKOUT' : 'GUEST CHECKOUT'}</span><h2>DELIVERY DETAILS</h2><p>{accountCustomer ? 'Your profile is ready. Select a saved address or update these checkout details.' : 'No account is required. Enter your shipping information to continue.'}</p></div>
        ...
        <FormField label="FULL NAME" name="name" value={customer.name} onChange={updateField} error={errors.name} autoComplete="name" />
        <div className="form-row"><FormField label="EMAIL ADDRESS" name="email" type="email" value={customer.email} onChange={updateField} error={errors.email} autoComplete="email" /><FormField label="WHATSAPP / PHONE" name="phone" type="tel" value={customer.phone} onChange={updateField} error={errors.phone} autoComplete="tel" /></div>
        <FormField label="SHIPPING ADDRESS" name="address" value={customer.address} onChange={updateField} error={errors.address} autoComplete="street-address" />
        <FormField label="CITY" name="city" value={customer.city} onChange={updateField} error={errors.city} autoComplete="address-level2" />
        <label className="form-field"><span>ORDER NOTES</span><textarea name="notes" value={customer.notes} onChange={updateField} rows="3" /></label>
      ...
      <div className="checkout-final-column">
        <CheckoutSummary entries={entries} quote={quote} />
        <PaymentOptionSelector value={paymentOption} onChange={changePayment} quote={quote} />
        <button className="btn btn--acid checkout-place-order" type="submit" disabled={submitting || !quote}>{submitting ? 'PLACING ORDER…' : 'PLACE ORDER'} <span>→</span></button>
        <a className="checkout-return-cart" href="/cart">← RETURN TO CART</a>
