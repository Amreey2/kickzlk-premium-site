    ...
    <section className="cart-section section-pad"><div className="container">
      ...
      {!catalog.loading && items.length > 0 && <div className="cart-layout reveal">
        ...
        <aside className="cart-summary"><span className="section-kicker">ORDER SUMMARY</span>
          ...
          <div className="cart-summary__rows"><div><span>Subtotal</span><strong>{formatProductPrice(quote?.subtotalAmount || 0)}</strong></div>{quote?.discountAmount > 0 && <div className="cart-discount"><span>Coupon discount</span><strong>− {formatProductPrice(quote.discountAmount)}</strong></div>}<div><span>Delivery</span><strong>CONFIRMED LATER</strong></div><div className="cart-summary__total"><span>Order Total</span><strong>{formatProductPrice(quote?.totalAmount || 0)}</strong></div><div><span>Pay Now</span><strong>{formatProductPrice(quote?.advanceAmount || 0)}</strong></div><div><span>Balance on Delivery</span><strong>{formatProductPrice(quote?.balanceAmount || 0)}</strong></div></div>
